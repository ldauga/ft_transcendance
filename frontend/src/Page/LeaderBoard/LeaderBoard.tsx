...
function LeaderBoard() {
	...
	useEffect(() => {
		if (!rows.length) {
			axiosConfig.get('https://10.64.1.68:5001/user').then(res => {
			...
			<div className="leaderboard">
				...
				<div className='tabContent'>
					<table>
						<tbody>
							{rows.map((row) => (
								<tr className='element' onClick={() => { history.pushState({}, '', window.URL.toString()); window.location.replace('https://10.64.1.68:3000/Profile/' + row.login) }} key={row.nickname}>
