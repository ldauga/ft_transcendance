...
function CreateInvitationRooms(props: { roomsConversData: { name: string, id: number }, setCreateInvitation: Function }) {
    ...
    const createInvitation = async () => {
        ...
        if (_user) {
            await axiosConfig.get('https://10.3.3.5:5001/user/login/' + _user.username).then(async (res) => {
                setInputValue("");
                ...
                if (res.data == "") {
                    ...
                    let a = 1;
                    let b = 1;
                    await axiosConfig.get('https://10.3.3.5:5001/invitationRequest/checkInvitationRequestForRooms/' + res.data.id + '/' + props.roomsConversData.name).then(async (res) => {
                        if (res.data == true) {
                        }
                        else {
                            a = 2;
                    ...
                    await axiosConfig.get('https://10.3.3.5:5001/participants/check/' + receiver_login_tmp + '/' + props.roomsConversData.name).then(async (res) => {
                        if (res.data == true) {
                        }
                        else {
                            b = 2;
