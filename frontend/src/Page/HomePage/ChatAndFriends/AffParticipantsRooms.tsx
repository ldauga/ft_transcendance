...
function AffParticipantsRooms(props: { roomsConversData: { name: string, id: number }, setAffParticipantsRooms: Function, setConversRooms: Function, closeConvers: Function, setRooms: Function, oldAffRoomConvers: string, setChat: Function }) {
    ...
    async function buttonMuteRoomParticipant() {
        ...
        await axiosConfig.get('https://10.3.2.5:5001/user/login/' + userToMute.login).then(async (res) => {
            if (res.data == "") {
                return;
            }
            else {
                let a = 1;
                let b = 1;
                await axiosConfig.get('https://10.3.2.5:5001/muteList/checkRoomMute/' + res.data.id + '/' + res.data.login + '/' + props.roomsConversData.name).then(async (res) => {
                    if (res.data == true) {
                    }
                    else {
                        a = 2;
    ...
    async function buttonBanRoomParticipant() {
        ...
        await axiosConfig.get('https://10.3.2.5:5001/user/login/' + userToBan.login).then(async (res) => {
            if (res.data == "") {
                return;
            }
            else {
                let a = 1;
                let b = 1;
                await axiosConfig.get('https://10.3.2.5:5001/blackList/checkRoomBan/' + res.data.id + '/' + res.data.login + '/' + props.roomsConversData.name).then(async (res) => {
                    if (res.data == true) {
                    }
                    else {
                        a = 2;
    ...
    async function buttonAddAdmin(item: { login: string, id: number, admin: boolean }) {
        let test = false;
        await axiosConfig.get('https://10.3.2.5:5001/user/login/' + item.login).then(async (res) => {
            if (res.data == "") {
                return;
            }
            else {
    ...
    async function buttonRemoveAdmin(item: { login: string, id: number, admin: boolean }) {
        ...
        await axiosConfig.get('https://10.3.2.5:5001/user/login/' + item.login).then(async (res) => {
            if (res.data == "") {
                ...
                await axiosConfig.get('https://10.3.2.5:5001/rooms/checkIfOwner/' + item.id + '/' + props.roomsConversData.name).then(async (res) => {
                    if (res.data == true) {
    ...
    const checkIfAdmin = async () => {
        let ifAdmin = false;
        await axiosConfig.get('https://10.3.2.5:5001/rooms/checkIfOwner/' + userData.userReducer.user?.id + '/' + props.roomsConversData.name, { withCredentials: true }).then(async (res) => {
            if (res.data == true) {
                setAdmin(true);
                ifAdmin = true;
            }
        })
        await axiosConfig.get('https://10.3.2.5:5001/participants/checkAdmin/' + userData.userReducer.user?.login + '/' + props.roomsConversData.name, { withCredentials: true }).then(async (res) => {
            if (res.data == true) {
                setAdmin(true);
                ifAdmin = true;
            }
    ...
    const getListItem = async (data: any) => {
        ...
        data.forEach((item: { id: number, login: string, nickname: string, profile_pic: string, admin: boolean, mute: boolean }) => {
            ...
            itemList.push(<div key={itemList.length.toString()} className='participant' onClick={(e) => { history.pushState({}, '', window.URL.toString()); window.location.replace('https://10.3.2.5:3000/Profile/' + item.login) }}>
