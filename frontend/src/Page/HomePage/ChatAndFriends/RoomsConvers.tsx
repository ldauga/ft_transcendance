...
function RoomsConvers(props: { setFriendList: Function, setRooms: Function, setRoomsConvers: Function, roomsConversData: { name: string, id: number }, oldAffRoomConvers: string, setChat: Function, setRoomsConversData: Function }) {
    ...
    const getUsers = async () => {
        ...
        setUsers([]);
        setParticipants([]);
        await axiosConfig.get('https://10.64.1.68:5001/participants/allUserForOneRoom/' + props.roomsConversData.name).then(async (res) => {
            res.data.forEach(async (item: { login: string, id: number }) => {
                await axiosConfig.get('https://10.64.1.68:5001/user/id/' + item.id).then(async (res) => {
                    if (i == 0) {
    ...
    const checkIfMute = async () => {
        await axiosConfig.get('https://10.64.1.68:5001/muteList/checkRoomMute/' + userData.userReducer.user?.id + '/' + userData.userReducer.user?.login + '/' + props.roomsConversData.name).then(async (res) => {
            if (res.data == true) {
                setMute(true);
