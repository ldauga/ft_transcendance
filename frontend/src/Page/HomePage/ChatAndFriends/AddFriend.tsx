...
function AddFriend(props: { setNewAddFriend: Function }) {
    ...
    async function buttonAddFriend() {
        ...
        if (userToSend) {
            await axiosConfig.get('https://10.64.1.68:5001/user/login/' + userToSend?.username).then(async (res) => {
                setInputValue("");
                ...
                if (res.data == "") {
                    ...
                    let b = 1;
                    let c = 1;
                    await axiosConfig.get('https://10.64.1.68:5001/invitationRequest/' + userData.userReducer.user?.id + '/' + res.data.id).then(async (res) => {
                        if (res.data == true) {
                        }
                        else {
                            a = 2;
                    ...
                    await axiosConfig.get('https://10.64.1.68:5001/friendList/' + userData.userReducer.user?.id + '/' + res.data.id).then(async (res) => {
                        if (res.data == true) {
                        }
                        else {
                            b = 2;
                    ...
                    await axiosConfig.get('https://10.64.1.68:5001/blackList/checkUserBan/' + userData.userReducer.user?.login + '/' + receiver_login_tmp).then(async (res) => {
                        if (res.data == true) {
