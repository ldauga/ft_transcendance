...
function FriendListItem(props: { setFriendList: Function, setConvers: Function, setConversCorrespondantData: Function, setOldAff: Function, closeFriendList: Function, item: { status: string, user: { id: number, login: string, nickname: string, profile_pic: string } }, setUpdate: Function }) {
    ...
    async function buttonBanUser() {
        let test = false;
        await axiosConfig.get('https://10.3.4.5:5001/user/login/' + props.item.user.login).then(async (res) => {
        ...
        return (
            ...
            >
                {props.item.status == 'in-game' ?
                    <MenuItem
                        onClick={() => { history.pushState({}, '', window.URL.toString()); window.location.replace('https://10.3.4.5:3000/Spectate/' + props.item.user.login) }}>
                        <ListItemIcon>
    ...
    return (
        <div className="inItemFriendList">
            <div className="inItemFriendList_left">
                <div className="friend-profile" onClick={() => { history.pushState({}, '', window.URL.toString()); window.location.replace('https://10.3.4.5:3000/Profile/' + props.item.user.login) }}>
