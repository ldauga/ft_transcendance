...
function RoomsList(props: { setRooms: Function, setRoomsList: Function, setChat: Function }) {
    ...
    const joinWithPassword = async () => {
        await axiosConfig.get('https://10.3.3.5:5001/rooms/' + userData.userReducer.user?.id + '/' + userData.userReducer.user?.login + '/' + roomToJoin.id + '/' + roomToJoin.name + '/' + password).then(async (res) => {
            if (res.data == "ok") {
                join(roomToJoin);
                setPassword("");
                setOpenDialog(false);
