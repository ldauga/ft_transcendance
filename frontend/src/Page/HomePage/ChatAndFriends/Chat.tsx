...
function Chat(props: { setFriendList: Function, setChat: Function, setConvers: Function, setConversCorrespondantData: Function, setRooms: Function, setOldAff: Function, setRoomsConvers: Function, setroomsConversData: Function, setOldAffRoomConvers: Function, closeChat: Function, setRoomsList: Function }) {
    ...
    const getListItem = async () => {
        ...
        await axiosConfig.get('https://10.3.2.5:5001/messages/' + userData.userReducer.user?.id + '/' + userData.userReducer.user?.id + '/' + userData.userReducer.user?.id).then(async (res) => {
            relationList = res.data;
        });
        if (relationList.length > 0) {
            for (let i = 0; i < relationList.length; i++) {
                if (!relationList[i].userOrRoom) {
                    const user = await axiosConfig.get('https://10.3.2.5:5001/user/id/' + relationList[i].id);
                    ...
                    const checkIfParticipant = await axiosConfig.get('https://10.3.2.5:5001/participants/check/' + userData.userReducer.user?.login + '/' + relationList[i].room_name);
