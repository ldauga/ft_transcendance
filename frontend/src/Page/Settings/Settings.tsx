...
function Settings() {
	...
	const handleClickOpen = (param: any) => {
		...
		if (param == setOpenEditZone2fa) {
			axiosConfig.get('https://10.3.2.5:5001/auth/2fa/generate/').then(res => (setUserParameter2FAQrCode(res.data)))
		}
	};

	const handleClose = (param: any) => {
		if (userParameterNewNickname != persistantReduceur.userReducer.user?.nickname)
			axiosConfig.post('https://10.3.2.5:5001/user/updateNickname', { nickname: userParameterNewNickname }).then((res) => { if (res.data) setUser(res.data) })
	...
	const sendGetRequest = (value: string) => {
		axiosConfig.get('https://10.3.2.5:5001/auth/2fa/turn-on/' + value)
			.then(res => {
				setTwoFactor(true);
				setUserParameter2FACode('');
				setUser(res.data);
						...
						{!persistantReduceur.userReducer.user?.isTwoFactorAuthenticationEnabled ?
							...
							<><h3>Desactivate 2FA :</h3>
								<div className='edit'>
									...
									<button onClick={() => { axiosConfig.get('https://10.3.2.5:5001/auth/2fa/turn-off/').then(res => { setUser(res.data) }) }}>Desactivate</button>
								</div>
							</>
						}
					</div>
