...
function NavBar(props: { openFriendConversFromProfile: boolean, dataFriendConversFromProfile: { id: number, login: string, nickname: string, profile_pic: string }, setOpenFriendConversFromProfile: Function }) {
	...
	const logout = () => {
		...
		delAllNotif();
		setTwoFactor(false);
		window.location.replace('https://10.3.2.5:3000')
	}

	async function findUserProfile() {
		if (searchBarContent) {
			let res = await axiosConfig.get('https://10.3.2.5:5001/user/login/' + searchBarContent);
			if (!res.data.login)
				res = await axiosConfig.get('https://10.3.2.5:5001/user/nickname/' + searchBarContent);
			if (!res.data.login)
				enqueueSnackbar('Cannot find user\'s profile.', { variant: "error", autoHideDuration: 2000 })
			else
				window.location.replace('https://10.3.2.5:3000/Profile/' + res.data.login);
		}
		return true;
	}
			...
			<nav>
				...
				<div className='right'>
					...
					<button className='reactour-notif' onClick={() => { //notifs
						console.log('nbNotif', persistantReducer.notifReducer.notifArray.length)
						console.log(persistantReducer.notifReducer.notifArray.length - lastNbNotif >= 0)
