...
function InvitationChecker(props: { children: any }) {
	...
	utilsData.socket.on('start_invite_game', function (info: { roomID: string, spectate: boolean }) {
		...
		window.location.replace('https://10.64.1.68:3000/Pong')
	});

	function verifInvitationRequest() {
		axiosConfig.get('https://10.64.1.68:5001/invitationRequest/' + persistantReducer.userReducer.user?.id/*, { withCredentials: true}*/).then((res) => {
			if (res.data.length) {
