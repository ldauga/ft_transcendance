...
function InvitationChecker(props: { children: any }) {
	...
	function verifInvitationRequest() {
		axiosConfig.get('http://10.3.3.5:5001/invitationRequest/' + persistantReducer.userReducer.user?.id/*, { withCredentials: true}*/).then((res) => {
			if (res.data.length) {
	...
	utilsData.socket.on('start_spectate', function (arrClient: Client[]) {
		...
		window.location.replace('http://10.3.3.5:3000/Pong')
	})
